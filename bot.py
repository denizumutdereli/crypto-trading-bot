...
class BinanceConnection:
    def __init__(self, file, sandbox, sync = False):
        self.time_offset = 0
        self.connect(file,sandbox)
        if sync:
            self.time_offset = self._get_time_offset()

    #Binance connection
    def connect(self, file, sandbox):
        lines = [line.rstrip('\n') for line in open(file)]
        key = lines[0]
        secret = lines[1]
        self.client = Client(key, secret)
        if sandbox == 1:
            self.client.API_URL = 'https://testnet.binance.vision/api' #test server
        else:
            play("alert")
            print('\n[black on white]Caution, you have selected real trade api option.[/]')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--balance', type=int, default=0, help='Display the balance')
    parser.add_argument('--wallet', type=int, default=1, help='Displays the coin balance')
    parser.add_argument('--sounds', type=int, default=0, help='Sounds Open:1 / Close:0')
    parser.add_argument('--tcoin', type=str, default='BNB', help='Trading coin. ie BNB')
    parser.add_argument('--lockLimit', type=int, default=100, help='Open pending orders quantity')
    parser.add_argument('--symbol', type=str, default='BTCBUSD', help='Trade symbol. ie. BTCBUSD')
    parser.add_argument('--interval', type=str, default='5m', help='Time interval')
    parser.add_argument('--maxTrade', type=int, default='20', help='Max trading volume')
    parser.add_argument('--contrade', type=int, default='1', help='Concurrent trading limit')
    parser.add_argument('--loop', type=int, default='10', help='Api update frame')
    parser.add_argument('--sandbox', type=int, default=1, help='Test api 1 / Real trade 0')
    ...
    parser.add_argument('--real', type=int, default=0, help='Real Trade 1 / Simulate Trade 0')
    ...
    filename = '.credentials.test' if args.sandbox == 1 else '.credentials.main'
    connection = BinanceConnection(filename, args.sandbox, True)

    tradingCoin = args.tcoin
    lockLimit = args.lockLimit
    symbol = args.symbol
    pairs = symbol+tradingCoin
    maxTrade = args.maxTrade 
    interval =args.interval
    limit = 500
    concurrentTradeLimit = args.contrade
    buyP = args.buy
    sellP = args.sell
    activeOrder = 0 #temporary
    real = args.real
   ...
   #account Info
    ...
    wallet = connection.client.get_asset_balance(asset=tradingCoin)
    ...
    while True:
        ...
        try:
            klines = connection.client.get_klines(symbol=pairs, interval=interval, limit=limit)
